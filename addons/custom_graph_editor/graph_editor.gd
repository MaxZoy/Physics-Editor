...
## Given a path to a .phgraph, returns a deserialized CGEGraph
## allowing to have just node and connectivity info and scrap out
## all the editor specific data.[br]
##
## [param path]: Path to the .phgraph file[br]
...
func _instantiate_file_dialog() -> void:
    ...
    const DEFAULT_FILE_NAME: String = "my_graph.phgraph"
    _file_dialog.current_file = DEFAULT_FILE_NAME

    _file_dialog.filters = PackedStringArray(["*.phgraph"])
